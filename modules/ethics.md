---
layout: page
title: "Module 4: Research Ethics"
permalink: /modules/ethics/
---

# Module 4: Research Ethics

This module provides a focused exploration of ethical considerations in research and scientific communication, co-taught with Dr. Sarah Reckess, JD (Upstate Medical University). You'll engage with real-world ethical dilemmas through case studies, debates, and interactive scenarios, developing the moral reasoning skills necessary to navigate complex ethical terrain in interdisciplinary research. The module moves from individual ethical principles to systemic pressures, then applies that framework to authorship, AI in research, misconduct, conflicts of interest, science communication, and intellectual property. Sessions are taught by Prof. Zhang, guest lecturer Eric Mathis (AI in research), and Dr. Reckess.

<div style="background: #f0f4ff; border-left: 4px solid #2c3e7a; padding: 16px 20px; border-radius: 0 6px 6px 0; margin: 20px 0;">
<strong>Student Profile (Mentimeter survey, n≈16):</strong> "Research Ethics" associations: <em>honesty</em>, <em>moral</em>, <em>integrity</em>, <em>responsibility</em>. Top ethical challenge faced: ensuring data accuracy (13/16). Lowest confidence: navigating authorship disputes (2.8/5). Reading compliance: only 6/14 read pre-class materials. Root cause of research problems per Alberts: inadequate ethical training (9/15), never-ending growth assumption (6/15).
</div>

---

## Module Structure: 7 Sessions + Final Presentations

| # | Session | Key Topics | Instructor |
|---|---------|------------|:---:|
| 1 | [Introduction to Ethics in Scientific Research](#session-1) | Ethics vs. law vs. morals; Resnik principles; systemic pressures | Dr. Reckess |
| 2 | [Authorship, Plagiarism & Peer Review](#session-2) | ICMJE criteria, CRediT taxonomy, self-plagiarism, authorship scenarios, AI authorship | Prof. Zhang |
| 3 | [Artificial Intelligence in Research](#session-3) | AI ghostwriting, hallucinations, detection, regulatory frameworks | Eric Mathis (guest) |
| 4 | [Research Misconduct](#session-4) | FFP, questionable practices, paper mills, responding to suspected violations | Dr. Reckess |
| 5 | [Conflicts of Interest & Industry Relationships](#session-5) | Individual and institutional COI, disclosure, mitigation | Dr. Reckess |
| 6 | [Science Communication and Science in Society](#session-6) | SciComm as ethics, public trust, genuine vs. false successes | Dr. Reckess |
| 7 | [Intellectual Property & Ownership](#session-7) | Patents, copyright, data sharing, ownership of discoveries | Dr. Reckess |
| — | [Final Team Presentations](#presentations) | Formative presentations; feedback informs the final report | All instructors |

### 📝 Running Assignment: Technology & Responsibility Report (Team)

The final team project runs alongside this module. Each team produces a professional report on a transformative technology:

| Session | Report Connection |
|:---:|---------------------|
| 1 | Ethical frameworks and systemic pressures → inform Section 4 (Ethical & Societal Analysis) |
| 2 | Authorship, plagiarism, and credit standards → inform Section 6 (Team Reflection on collaborative process) |
| 3 | AI ethics → applicable to all topics; also informs how teams use AI in their own writing |
| 4–5 | Misconduct and conflicts of interest → inform Section 4 |
| 6 | Science communication principles → inform Section 5 (Recommendations for Responsible Stewardship) |
| 7 | IP and ownership → inform Section 6 |
| — | Presentations on December 7 are formative: feedback informs revision before the December 15 submission |

**Topic choices:** mRNA Technology, Advanced Robotics & Autonomous Systems, CRISPR & Gene Editing, Nuclear Technology, Synthetic Biology, Neurotechnology & Brain-Computer Interfaces, Quantum Computing.

---

<details open>
<summary><h3 style="display:inline; cursor:pointer;" id="session-1">Session 1: Introduction to Ethics in Scientific Research</h3></summary>

**Goal:** Establish the distinction between ethics, law, and moral values. Apply Resnik's ethical principles to real scenarios, then examine how systemic pressures (Alberts et al.) create the conditions for ethical failures.

#### 📊 What Your Survey Told Us

- **"Research Ethics" associations:** "Honesty," "moral," and "integrity" dominated the word cloud — strong individual-level framing. Notably absent: systemic terms like "incentives," "funding pressure," or "institutional culture." This session bridges that gap.
- **Most challenging ethical consideration faced:** Ensuring data accuracy (13/16) was the clear leader, followed by citing sources/plagiarism (5) and collaborator fairness/credit (5). Only 1 student cited safety concerns and 1 said they hadn't encountered any.
- **Confidence levels:**

| Skill | Confidence (1–5) |
|-------|:---:|
| Identify a potential conflict of interest | **3.9** |
| Handle a labmate behaving unethically | **3.5** |
| Navigate authorship disputes on a paper | **2.8** |

That 2.8 on authorship is the lowest confidence score in any module — Session 2 (Authorship, Plagiarism & Peer Review) addresses it directly.

- **Reading compliance:** 6/14 read the materials. 8 did not. This is itself an ethics teaching moment about preparation and accountability.

#### 📖 Pre-Class Reading

- 📄 [**Resnik, D. "What Is Ethics in Research & Why Is It Important?"**](https://www.niehs.nih.gov/research/resources/bioethics/whatis) — NIEHS (2020)
  > Foundational framework: honesty, objectivity, integrity, carefulness, openness, respect, social responsibility.
- 📄 [**Alberts, B. et al. "Rescuing U.S. Biomedical Research from Its Systemic Flaws"**](https://www.pnas.org/doi/10.1073/pnas.1404402111) — PNAS (2014)
  > The structural argument: hypercompetition, perverse incentives, and inadequate training create conditions for ethical failures.

#### 🎯 In-Class Activities (~80 min)

**Mentimeter warm-up:** "What's the first word that comes to mind when you hear 'Research Ethics'?" Reveal word cloud. Discuss: why are most responses about *individual* character, not *systemic* conditions?

**Mini-lecture: Ethics vs. Law vs. Morals:** Establish the framework. Ethics is not just "following the rules" — it's reasoning about what *should* be done when rules are ambiguous or conflicting.

**Alberts deep dive:** Your survey showed 9/15 identified "inadequate ethical training" as the root cause, while 6 chose "assumption of never-ending growth." Discuss: Alberts actually argues the growth assumption is root — the training gap is a *symptom*. What changes if you accept that framing?

**Mentimeter: Resnik principles under pressure:** Rank which principles are most threatened by hypercompetition. Your results: (1) Honesty, (2) Responsible Publication, (3) Openness, (4) Objectivity. Discuss why honesty ranked #1 — is it because it's the most *valued* or the most *vulnerable*?

**Perverse incentives exercise:** Open-ended: "Name one perverse incentive that could lead to ethical deviation." Your responses: "publish or die," "quantity over quality," "pressure to obtain funding," "publication bias," "overstating results." Connect each to specific Resnik principles it threatens.

**Closing poll:** "What is the most significant barrier to ethical research?" and "What is one action that could help create a more ethical research culture?"

#### 💡 Key Takeaways

- Ethics is not just about bad individuals — systemic pressures create conditions where good people make bad choices
- Resnik's principles provide a vocabulary for ethical reasoning; Alberts provides the structural context
- Your instinct that "honesty" is most at risk is well-supported — hypercompetition incentivizes exaggeration, selective reporting, and premature publication
- The 8/14 who didn't read: this is itself an ethics question about preparation and professional responsibility

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-2">Session 2: Authorship, Plagiarism & Peer Review</h3></summary>

**Goal:** Explore who qualifies as an author, how credit disputes arise, what self-plagiarism is and why it matters, what the peer review process expects of authors and reviewers, and how emerging tools (CRediT taxonomy, AI authorship policies) are reshaping these conversations. Directly addresses the lowest confidence score in the module (2.8/5 on authorship disputes).

#### 📖 Pre-Class Reading

- 📄 [**ICMJE: Defining the Role of Authors and Contributors**](https://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html) — The four criteria for authorship
- 📄 [**CRediT: Contributor Roles Taxonomy**](https://credit.niso.org/) — The 14-role taxonomy for transparent contribution attribution
- 📄 [**ORI: "Avoiding Plagiarism, Self-Plagiarism, and Other Questionable Writing Practices: A Guide to Ethical Writing"**](https://ori.hhs.gov/avoiding-plagiarism-self-plagiarism-and-other-questionable-writing-practices-guide-ethical-writing) — the assigned self-plagiarism reading (the older iThenticate white paper link is no longer live; the ORI guide covers the same ground from the federal research-integrity office)

#### 🎯 In-Class Activities (~80 min)

**Mentimeter recall:** Your confidence in navigating authorship disputes was 2.8/5 at the start of the module. Let's see if we can raise that today.

**Mini-lecture: ICMJE criteria + CRediT:** The four ICMJE criteria (substantial contribution, drafting/revising, final approval, accountability). Why all four must be met. The CRediT taxonomy as a complementary tool that makes contribution types visible without gatekeeping authorship. Key question: under these criteria, can AI be an author? (Most journals now say no — but the reasoning matters.)

**Scenario-based discussion:** Four authorship dilemmas — teams discuss and vote:
- A PI contributes the idea and funding but doesn't write or revise. Author?
- A technician runs all experiments but doesn't participate in analysis or writing. Author?
- A collaborator provides a critical reagent and protocol but doesn't review the manuscript. Author?
- A graduate student writes the paper but their advisor insists on first authorship. What do you do?

**Bridge from Module 1:** Reopen Appendix C of the Team Charter. The transistor/BCS comparison separated contribution, leadership, paper authorship/order, patent inventorship, ownership, and prize recognition; it did not decide that all team credit is an inventorship question. This session now applies ethical and publication standards to author eligibility, order, and accountability. Patent inventorship, ownership, commercialization, and the CRISPR dispute are reserved for Session 7 (Intellectual Property & Ownership).

**Self-plagiarism:** Open with the ORI guide and the classic framing question: "To what extent do you agree: self-plagiarism is a victimless crime because I'm only reusing my own work?" Then sort the cases: republishing a conference paper without new analysis, reusing methods with citation, publishing one study as several "salami-sliced" papers, and quoting your own thesis with citation. Apply it to authorship: is republishing your own conference paper in a journal (with minor revisions) ethical if you disclose it? The nuance: copyright transfer agreements may make your own words legally someone else's property.

**Peer review expectations:** What Module 3 already showed: reviewers who quote, locate, and answer comments; authors who respond point by point. Now ask the ethical question: what do authors owe reviewers, and reviewers owe authors, beyond the workflow? Confidentiality, conflict disclosure, and not using review to claim priority.

**AI authorship exercise:** Take a paragraph from your Technology & Responsibility Report. If AI helped draft it, apply the ICMJE criteria: did AI make a "substantial contribution"? Can AI approve the final version? Can AI be accountable? What does this mean for your disclosure obligations?

**Closing:** Mentimeter: "How confident are you now in navigating authorship disputes?" Compare with 2.8 baseline.

#### 💡 Key Takeaways

- ICMJE criteria are an influential biomedical-journal standard; apply them carefully and also check the target journal's and field's requirements
- CRediT taxonomy makes invisible contributions visible — advocate for its use in your publications
- AI cannot meet ICMJE criteria (especially accountability) — but your obligation to disclose AI assistance is real
- Undiscussed assumptions create authorship risk; the Module 1 provisional plan makes expectations revisable and visible but does not by itself settle eligibility, order, accountability, or power concerns
- Self-plagiarism has genuine nuance: reusing methods descriptions ≠ republishing the same study

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-3">Session 3: Artificial Intelligence in Research</h3></summary>

**Goal:** Examine AI's ethical implications in scientific research — from ghostwriting to hallucinations to detection failures — building on what students already experienced in Modules 2 and 3.

#### 📖 Pre-Class Reading (Core — all students)

- 📄 [**Gerhard, D. "Detection or Deception: The Double-Edged Sword of AI Detection in Research Misconduct"**](https://www.the-scientist.com/detection-or-deception-the-double-edged-sword-of-ai-in-research-misconduct-72354) — *The Scientist* (Dec 2024)
- 📄 [**Lin, Z. "AI Chatbots Are Already Biasing Research — We Must Establish Guidelines"**](https://www.nature.com/articles/d41586-025-02810-5) — *Nature* (2025)
- 📄 [**Brainard, J. "Far More Authors Use AI to Write Science Papers Than Admit It"**](https://www.science.org/content/article/far-more-authors-use-ai-write-science-papers-admit-it-publisher-reports) — *Science* (2025)

#### 📖 Optional Reading (consult as interested)

- 📄 [**Frankel, F. "Will AI Jeopardize Science Photography?"**](https://www.nature.com/articles/d41586-025-00532-2) — *Nature* (2025)
- 📄 ["Smart Biology on a Budget: Why Researchers Should Leverage AI to Rethink Experimental Design"](https://www.the-scientist.com/smart-biology-on-a-budget-why-researchers-should-leverage-ai-to-rethink-experimental-design-73208) — *The Scientist* (2025)
- 📄 [**Suzgun, M. et al. "Language Models Cannot Reliably Distinguish Belief from Knowledge and Fact"**](https://doi.org/10.1038/s42256-025-01113-8) — *Nature Machine Intelligence* (2025)

#### 🎯 In-Class Activities (~80 min)

**Bridge from Modules 1–3:** Module 1 introduced disclosure and verification, with an optional small local detection exercise; Module 2 examined AI in the research enterprise; and Module 3 evaluated AI feedback on scientific writing. Revisit what each activity actually established and its limits. Those were *capability and workflow* questions. Today we ask the *ethical* questions.

**The ghostwriting problem:** Brainard reports far more authors use AI than disclose it. Discussion: Is undisclosed AI writing assistance the same as ghostwriting? As plagiarism? As a new category we don't have a name for yet? Where is the line between "AI as spell-checker" and "AI as co-author"?

**AI bias case study:** Lin's Nature piece argues AI chatbots are already biasing research directions. If AI tools steer researchers toward certain hypotheses or methods, who is responsible for the resulting bias? The AI developer? The researcher? The institution that mandates AI tool use?

**Detection paradox discussion:** Use Gerhard's "double-edged sword" analysis. If the optional Module 1 exercise was used, its three model configurations scored 0–20% on one small classroom task; restate its methods and limits rather than generalizing it to every detector. What risks arise when policy depends mainly on detection? Teams propose complementary accountability mechanisms.

**Regulatory frameworks mini-debate:** Should AI in research be regulated through existing research integrity frameworks (extend current rules), through new AI-specific legislation (create new rules), or through professional self-regulation (scientists police themselves)? Teams take assigned positions.

**Closing: Where do YOU draw the line?:** Poll Everywhere scale: Rate the ethical acceptability of these AI uses (1 = clearly unethical, 5 = clearly acceptable): using AI to check grammar, using AI to restructure an argument, using AI to draft a literature review section, using AI to generate hypotheses, using AI to write an entire methods section.

> 🔗 **Connection to Module 3:** In Module 3 Lecture 6, you audited AI feedback on a public abstract and verified every factual claim in the output — some claims failed or resisted verification. Today's session asks: given those limitations, what are the ethical obligations of researchers who use these tools? Resnik's principle of honesty applies directly — presenting AI-generated content as your own work violates it, regardless of detection capability.

#### 💡 Key Takeaways

- Undisclosed AI use in writing is an integrity issue, not a technology issue — existing ethics frameworks apply
- AI-detection systems have important limits — accountability cannot depend on detection alone and also requires transparency, verification, and professional norms
- AI bias in research is a systemic risk that extends Alberts' perverse incentives argument into new territory
- The ethical line isn't fixed — it depends on disclosure, the type of contribution, and the stakes of the research

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-4">Session 4: Research Misconduct</h3></summary>

**Goal:** Define the three types of research misconduct (fabrication, falsification, plagiarism), explore the gray zone of "questionable research practices," examine the paper-mill case, and apply the National Academies' guidance on responding to suspected violations.

#### 📖 Pre-Class Reading

- 📄 Marcus, A. & Oransky, I. "An Epidemic of Scientific Fakery Threatens to Overwhelm Publishers" — *Washington Post* (June 11, 2024) — PDF on Blackboard
- 📄 [**National Academies. "Responding to Suspected Violations of Professional Standards"**](https://www.ncbi.nlm.nih.gov/books/NBK214564/) — chapter of *On Being a Scientist: A Guide to Responsible Conduct in Research*, 3rd ed. (2009), freely available on NCBI Bookshelf

#### 🎯 In-Class Activities (~80 min)

**Mentimeter: Misconduct or gray zone?:** Classify each example as fabrication, falsification, plagiarism, a questionable research practice, or acceptable: cleaning a figure "for clarity," dropping an outlier without reporting it, copying a colleague's methods paragraph, padding a reference list, splitting one study into three papers.

**Mini-lecture: FFP + Questionable Practices:** The bright line (fabrication, falsification, plagiarism) vs. the gray zone (p-hacking, selective reporting, honorary authorship, salami-slicing). Most misconduct exists in the gray zone — these are the decisions students will actually face.

**Paper mills case study:** The Washington Post article describes an "epidemic" of fraudulent papers. Discussion: What is the single biggest driver? Then rank solutions from most to least impactful: strengthen government oversight, abolish citation counts as quality metric, retire pay-for-play journal model.

**Responding to suspected violations:** From the National Academies chapter: what should you do if you suspect a colleague of misconduct — or if someone raises a concern about you? Discuss the reporting routes, the difference between asking a question and making an accusation, and what a fair institutional process looks like. Relate to the Module 1 Charter: what does your own team's escalation clause do and not do?

**Closing: Most powerful change:** Open-ended: "Based on all the readings, what is the most powerful change we could make to reduce research misconduct?"

**Exit ticket:** Name one gray-zone practice you have personally seen or heard described, and classify it against the FFP/QPR boundary.

**Contingency reserve**

> 🔗 **Connection to Module 3:** In Module 3 Lecture 4, you discussed transparent peer review and how it affects accountability. Misconduct thrives in opacity — the same pressures that make transparent review uncomfortable also make it valuable.

#### 💡 Key Takeaways

- Most ethical failures aren't dramatic fraud — they're "questionable research practices" in gray zones
- Paper mills are a systemic problem driven by the same perverse incentives Alberts identified
- A fair response to suspected misconduct protects both the reporter and the accused — process matters as much as outcome
- The bright line matters precisely because the gray zone is where most real decisions live

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-5">Session 5: Conflicts of Interest & Industry Relationships</h3></summary>

**Goal:** Identify different types of conflicts of interest — individual and institutional — articulate the risks they create for scientific validity and public trust, and practice mitigation strategies.

#### 📖 Pre-Class Reading

- 📄 [**Resnik, D.B. "Institutional Conflicts of Interest in Academic Research"**](https://doi.org/10.1007/s11948-015-9702-9) — *Science and Engineering Ethics* 25 (2019): 1661–1669. Free full text via [PMC4824668](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4824668/)

#### 🎯 In-Class Activities (~80 min)

**Mini-lecture: Individual vs. institutional COI:** A researcher's financial interest is only one layer. The Resnik reading extends the question to institutions: when a university receives industry funding, runs its own clinical trials, or holds equity in spinoffs, whose judgment is compromised — and who is supposed to notice?

**COI scenario analysis:** Four scenarios, rated on a 1–5 "how ethically problematic" scale:
- A professor holds $50K in pharmaceutical stock while running a clinical trial for that company
- A researcher accepts a $300 "finder's fee" per patient recruited into a company's trial
- A university accepts a multi-million dollar gift from a tech company for a new research center
- A public health professor consults for a soda company on "healthier product" initiatives

Teams discuss: Where is the line between acceptable and unacceptable? Does disclosure solve the problem, or just acknowledge it?

**Institutional COI discussion:** From the Resnik reading: university-industry research centers, institutional equity in spinoffs, and the special problem that institutional COI can affect every researcher in the institution at once. Who can credibly oversee an institutional conflict when the institution itself is the conflicted party?

**Mitigation strategies:** Teams draft a management plan for one scenario: what combination of disclosure, recusal, independent review, and structural separation would make the arrangement defensible? Test each plan: would it survive a headline?

**Closing discussion:** "What is the difference between having a conflict of interest and letting one affect your judgment?" Connect to Session 1: COI is one of the systemic pressures Alberts described.

**Exit ticket:** One scenario from today whose management plan you would trust, and the single element that makes it defensible.

#### 💡 Key Takeaways

- Conflicts of interest aren't inherently unethical — but undisclosed COI is always problematic
- Disclosure is necessary but not sufficient: management plans, recusal, and independent review do the work disclosure only announces
- Institutional COI is harder to see and harder to oversee than individual COI
- The same pressure that makes COI dangerous — dependence on external resources — is built into the modern research enterprise

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-6">Session 6: Science Communication and Science in Society</h3></summary>

**Goal:** Frame science communication not as a practical skill but as an ethical obligation grounded in engineering and scientific ethics codes (NSPE, ABET, Resnik). Analyze why some communications succeed and others fail, using your own survey data as evidence.

#### 📊 What Your Survey Told Us

This session produced the richest survey data in the module:

- **Where you learn about discoveries:** "literature" and "papers" dominated — you rely on primary sources, not media.
- **Where your *family* learns about discoveries:** Social media (11/21), TV news (6), newspapers/websites (4), podcasts (4). Only 1 said "directly from scientists." And 4 said their family doesn't seek out science news at all. This gap between how *you* consume science and how *the public* consumes science is the ethical problem.
- **Science news outlets you read:** Nature News (13/17), Science News (5), NYT Science (5), Physics Today (5). Only 1–2 read Scientific American or the Atlantic — the outlets that actually bridge to general audiences.
- **Public trust perception:** You estimated 3.5/5 for how much the public trusts scientists. The Pew data shows it's more nuanced and declining.
- **% of Americans who think scientists are good communicators:** Most of you (7/16) guessed 45%. The actual Pew number reveals a disconnect between how scientists see themselves and how the public sees them.
- **Biggest barriers to sci-comm:** "Don't know where/how to start" (12/15) was the dominant answer, followed by "don't have time" (6). Only 1 cited "lack of confidence" — the barrier is practical, not psychological.
- **Platforms used/considered:** LinkedIn (10), academic platforms like ResearchGate (9), but 7/19 don't promote their research outside journals at all. TikTok: 0.
- **Scientists in policy:** 10/15 said yes, actively engage. 6 said "it depends." 0 said no.
- **Misleading press release (coffee reduces cancer 40% — but study was in mice):** 14/15 identified "misleading headline" (Rule 4) as the primary problem. 8 noted it "didn't acknowledge limitations" (Rule 9).
- **Preliminary harmful supplement finding + media contact:** 9/15 said decline and wait for peer review. 5 said share but emphasize it's preliminary. Only 1 said share fully.

**Your examples of successful sci-comm:** cigarettes/anti-smoking, ozone depletion, personal hygiene, weather forecasts, Silent Spring, vaccines, Ig Nobel Prizes, Startalk podcast.

**Your examples of unsuccessful sci-comm:** vaccines cause autism, COVID-19 vaccines, climate change, Tuskegee studies, nuclear bomb, Radium girls, pandemic masking guidelines, GLP-1, chatgpt.

> **The productive surprise:** Several of your "successes" (vaccines, COVID-19) also appear on your "failures" list — suggesting students already sense that communication outcomes are more complex than simple success/failure categories. This is exactly where we dig in.

#### 📖 Pre-Class Reading

- 📄 [**Borowiec, B.G. "Ten Simple Rules for Scientists Engaging in Science Communication"**](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1011251) — PLOS Computational Biology (2023)
- 📄 [**Tyson, A. & Kennedy, B. "Public Trust in Scientists and Views on Their Role in Policymaking"**](https://www.pewresearch.org/science/2024/11/14/public-trust-in-scientists-and-views-on-their-role-in-policymaking/) — Pew Research Center (Nov 2024)
- 📄 [**Elliott, K.C. & Resnik, D.B. "Making Open Science Work for Science and Society"**](https://doi.org/10.1289/EHP4808) — *Environmental Health Perspectives* (2019), open access

#### 🎯 In-Class Activities (~80 min)

**Mentimeter: Discovery sources:** Reveal the gap between where *you* learn about science (literature, papers) and where your *family* learns (social media, TV). This is the communication problem in one data point.

**The ethical obligation argument:** Science communication is not optional. NSPE Code of Ethics requires engineers to "hold paramount the safety, health, and welfare of the public." ABET requires graduates who can "communicate effectively with a range of audiences." Resnik's "social responsibility" principle demands it. If publicly funded research doesn't reach the public, is that an ethical failure?

**Genuine successes vs. false successes:** Analyze your examples. Genuine successes (seatbelts, iodized salt, handwashing, ozone/CFCs) share traits: non-politicized, simple actionable message, clear personal relevance, no powerful opposition, sustained institutional support. Many commonly cited "successes" (climate change, GMOs, mRNA vaccines) are actually mixed or failed cases when measured by public behavior change. The vaccine/COVID appearing on *both* your lists proves this point.

**Press release analysis:** The coffee/cancer press release. You correctly identified the misleading headline (14/15). Now rewrite it: produce a headline that is both accurate *and* compelling. Teams compete. Discuss: Is it possible to be accurate without being boring?

**Preliminary findings scenario:** You were split (9 decline, 5 share with caveats). Discuss: What if the supplement is widely used? Does urgency change the ethical calculus? Connect to Resnik's principle of social responsibility vs. responsible publication.

**Closing: Barriers and next steps:** Your #1 barrier was "don't know where/how to start" (12/15). Brief overview of concrete starting points: lab website, LinkedIn posts summarizing papers, departmental newsletters, local media contacts. The barrier is practical, and practical barriers have practical solutions.

> 🔗 **Connection to Module 3:** In Module 3 Lecture 4, you examined transparent peer review and open access. The same principle applies here — making science accessible is an ethical obligation grounded in the same transparency values you identified as benefits of open review.

#### 💡 Key Takeaways

- Science communication is an ethical obligation, not a career bonus — professional codes require it
- The gap between how scientists consume information and how the public does is the core problem
- Genuine communication successes share identifiable traits; many commonly cited "successes" are actually mixed cases
- Your biggest barrier ("don't know where to start") is solvable — the ethical failure is choosing not to start

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="session-7">Session 7: Intellectual Property & Ownership</h3></summary>

**Goal:** Understand the four types of intellectual property and how they impact scientific research output, data sharing practices, and the ownership of discoveries.

#### 📖 Pre-Class Reading

- 📄 *On Being a Scientist*: Chapter on Intellectual Property — PDF on Blackboard
- 📄 [**Brown, W.M. "Intellectual Property Law: A Primer for Scientists"**](https://doi.org/10.1385/MB:23:3:213) — *Molecular Biotechnology* 23 (2003): 213–224

#### 🎯 In-Class Activities (~80 min)

**Mini-lecture: Four types of IP:** Patents, copyrights, trade secrets, trademarks — each with different implications for scientific research. Key distinctions: patents protect inventions (but require disclosure), copyright protects expression (but not ideas), trade secrets protect information (but require secrecy). How each creates different incentive structures for openness vs. protection.

**Patent case study:** Introduce the CRISPR patent dispute. The Broad Institute holds key U.S. patents despite Doudna/Charpentier winning the Nobel. Analyze the legal record and current source packet before discussing: What does patent law recognize, how does that differ from scientific credit, and should fundamental biological discoveries be patentable at all?

**Data sharing dilemma:** The Human Genome Project's "Bermuda Agreement" (rapid public data release) vs. Celera's proprietary model. Analyze the source record and the public/private incentives here as an open-science and data-governance case; Module 1 no longer previews this comparison. Modern parallel: Should AI training data be open? Should datasets funded by public grants be freely available?

**IP in your research:** Each student identifies one potential IP issue in their own research: Does your university have a technology transfer office? Who owns your thesis data? If you develop code or a method, who controls it? What happens if you leave your lab?

**Closing discussion:** "What is the right balance between protecting intellectual property and sharing knowledge openly?" Connect to the full module arc: Resnik's openness principle, Alberts' critique of competition, and science communication's obligation to the public.

#### 💡 Key Takeaways

- IP law and ethical norms often conflict — knowing both gives you the ability to navigate real situations
- The CRISPR case shows that scientific credit and legal ownership can be entirely separate
- Data sharing norms are field-specific and evolving — know your field's expectations before you need them
- Everything in this module connects: individual ethics (Resnik) → systemic pressures (Alberts) → communication obligations → AI challenges → credit and ownership

---

</details>

<details>
<summary><h3 style="display:inline; cursor:pointer;" id="presentations">Final Team Presentations (December 7)</h3></summary>

**Goal:** Formative presentations of each team's draft Technology & Responsibility Report, followed by structured feedback. Presentations are not graded on polish; they exist so feedback can inform the final report due December 15.

**Format:** Each team presents its technology, the ethical analysis, and its draft recommendations (approximately 8 minutes per team plus questions). Other teams and all instructors provide feedback against the report rubric.

</details>

---

## 📚 Additional Resources

### Ethics Frameworks

- 📄 [**The Belmont Report**](https://www.hhs.gov/ohrp/regulations-and-policy/belmont-report/read-the-belmont-report/index.html) — Respect for persons, beneficence, justice
- 📄 [**NSF Responsible Conduct of Research Guidelines**](https://www.nsf.gov/bfa/dias/policy/rcr.jsp)
- 📄 [**COPE: Ethical Guidelines for Peer Reviewers**](https://publicationethics.org/files/Ethical_Guidelines_For_Peer_Reviewers.pdf)

### Dual-Use & Biosecurity

- 📄 [**U.S. Government DURC Oversight Policy**](https://aspr.hhs.gov/S3/Pages/Dual-Use-Research-of-Concern-Oversight-Policy-Framework.aspx)
- 📄 [**NAS: Biosecurity Challenges of High-Containment Labs**](https://nap.nationalacademies.org/catalog/11330/biosecurity-challenges-of-the-global-expansion-of-high-containment-biological)

### Bioinspired Ethics (Supplementary)

- 📄 [**"Biomimethics: Ethical Challenges in Bioinspired Innovation"**](https://doi.org/10.1088/1748-3190/ace7a2) — *Bioinspiration & Biomimetics* (2023)
- 📄 [**"Ethical, Legal, and Social Implications of Bio-Integrated Devices"**](https://www.cell.com/device/fulltext/S2666-9986(25)00154-1) — *Device* (2025)
- 📄 [**EU: Ethics Guidelines for Trustworthy AI**](https://digital-strategy.ec.europa.eu/en/library/ethics-guidelines-trustworthy-ai)

### Authorship & Publication

- 📄 [**ICMJE Authorship Criteria**](https://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html)
- 📄 [**CRediT Contributor Roles Taxonomy**](https://credit.niso.org/)

---

## 📚 Module Activities & Learning Objectives

Through seven content sessions and a final presentation day, students develop skills in:

- **Ethical Framework Application:** Using Resnik's principles and Alberts' systemic analysis to reason about real research dilemmas
- **Research Integrity:** Identifying fabrication, falsification, plagiarism, and questionable research practices — including gray-zone decisions students will actually face
- **Authorship & Credit:** Applying ICMJE criteria, using CRediT taxonomy, and navigating credit disputes
- **AI Ethics in Research:** Navigating disclosure obligations, bias risks, and regulatory questions around AI tools
- **Conflicts of Interest:** Recognizing individual and institutional COI and designing defensible mitigation plans
- **Science Communication Ethics:** Understanding communication as an ethical obligation grounded in professional codes, not just a career benefit
- **Intellectual Property:** Distinguishing patents, copyright, trade secrets, and trademarks, and navigating data ownership and sharing

**» Detailed case studies, assignment instructions, and submission portals are available on the course Blackboard site.**

---

### Archived Session (not offered Fall 2026): Dual-Use Research

<details>
<summary><h3 style="display:inline; cursor:pointer;">Dual-Use Research — preserved for future offerings</h3></summary>

**Goal:** Identify the inherent tension between scientific openness and security, articulate the formal definition of DURC, and evaluate oversight mechanisms.

#### 📖 Pre-Class Reading

- 📄 [**National Academies: "Dual Use Research of Concern in the Life Sciences" (Introduction)**](https://doi.org/10.17226/24761) — pp. 9–22
- 📄 [**Zoloth, L. "For the Sake of 600,000 Children, Science Must Be Bold"**](https://www.nytimes.com/2024/12/15/opinion/malaria-genetics-disease.html) — *NY Times* (Dec 2024)
- 📄 [**U.S. Government Policy for Institutional Oversight of Life Sciences DURC**](https://aspr.hhs.gov/S3/Pages/Dual-Use-Research-of-Concern-Oversight-Policy-Framework.aspx) (2015) — pp. 1–19

#### 📊 What Your Survey Told Us

- **DURC definition:** 10/10 correctly identified "research that could be directly misapplied to cause widespread harm." Strong conceptual grasp after the reading.
- **Values in tension:** Students identified "transparency and security," "scientific openness and [security]," and "novelty" as the conflicting values. The core dual-use tension is precisely this: openness enables progress *and* misuse.
- **Concern levels for specific scenarios:**

| Scenario | Concern (1–5) |
|----------|:---:|
| Engineering a crop pathogen resistant to all pesticides | **4.7** |
| H5N1 transmissibility study in ferrets | **4.1** |
| Publishing full genome of a highly pathogenic agent | **3.4** |
| Reconstituting extinct smallpox virus for vaccines | **3.3** |

The crop pathogen scored highest — interesting because it's the least discussed in popular media but arguably the most strategically concerning.

- **DURC oversight ranking (most to least effective):** (1) Federal agency review, (2) Institutional Review Entities, (3) Education/training, (4) Restricting publication. Students trust top-down oversight most, but rank education higher than censorship.

#### 🎯 In-Class Activities (~80 min)

**Mentimeter: Values in tension:** "What two values are in conflict in dual-use research?" Reveal responses. Frame the session around this tension.

**Mini-lecture: DURC framework:** Formal definition, the 15 categories of DURC agents, institutional oversight structure (IBC, IRE, federal review). The H5N1 ferret study as the canonical case.

**Scenario analysis:** Rate concern for the 4 scenarios above. Reveal results. Deep discussion on the crop pathogen: Why did this score highest? Is agricultural bioterrorism under-regulated compared to human pathogens?

**Zoloth case discussion:** The malaria gene drive argument — when does the moral imperative to act override the precautionary principle? Teams debate: "Should scientists proceed with gene drive research for malaria even if we can't fully predict ecological consequences?"

**Oversight mechanism ranking:** Reveal the ranking. Discuss: If education ranked #3, why is it the mechanism most of you will actually encounter (like this course)? Is that a problem?

#### 💡 Key Takeaways

- Dual-use isn't a binary — it's a spectrum of risk that requires ongoing assessment
- The strongest oversight combines institutional review with researcher education — neither alone is sufficient
- Agricultural biosecurity is a blind spot in public discourse despite scoring highest concern
- The tension between openness and security has no permanent resolution — only ongoing judgment

</details>

---

<div class="module-nav" style="margin-right: 30px;">
  <a href="../paper/" class="btn" style="margin-right: 40px;">← Scientific Writing & Peer Review</a>
  <a href="../../" class="btn" style="margin-right: 40px;">Course Home →</a>
</div>
